...
public class BasePage {
    ...
    protected int selectDropDownListRandomOptionByIdWithJS(String message, ID id) {
        ...
        return Integer.parseInt(((JavascriptExecutor) driver).executeScript(js).toString());
    }
     ...
     */
    protected boolean
    isCheckboxChecked(Locator locator, Object... args) {
        return getElement(locator, args).isSelected();
    }

    protected void setCheckboxState(String message, boolean checked, Locator locator,Locator locator2, Object... args) {
        if (checked ^ isCheckboxChecked(locator, args)) {
            click(message, locator2, args);
        }
    }
    ...
    protected boolean isElementPresent(String message, Locator locator, Object... args) {
        Reporter.log(message);
        return (getElementsCount(locator, args) > 0);
    }
    ...
    protected void switchToJustOpenedTab() {
    ...
    public void switchToJustOpenedWindow() {
        String winHandleBefore = driver.getWindowHandle();
        for (String winHandle : driver.getWindowHandles()) {
            driver.switchTo().window(winHandle);
        }
    ...
    protected void waitForElementToBeClickable(Locator locator, Object... args) {
        By by = locator.get(args);
        WebDriverWait wait = new WebDriverWait(driver, ConstantsPma2.ELEMENT_TIMEOUT_SECONDS);
        ...
        do {
            ...
            delta = System.currentTimeMillis() - startTime;
                ...
                && (delta <= ConstantsPma2.ELEMENT_TIMEOUT_SECONDS * 1000));

        if (delta > ConstantsPma2.ELEMENT_TIMEOUT_SECONDS * 1000) {
            throw new InvalidElementStateException("Element did not stand at the same place for " + ConstantsPma2.ELEMENT_TIMEOUT_SECONDS + " seconds");
        }
    ...
    protected void waitForElementContainsText(String text, Locator locator, Object... args) {
        ...
        while (System.currentTimeMillis() - millis < ConstantsPma2.ELEMENT_TIMEOUT_SECONDS * 1000) {
    ...
    protected void mouseMove(String message, Locator locator, Object... args) {
        Reporter.log(message);
//        moveMouseOut();
        new Actions(driver).moveToElement(getElement(locator, args)).build().perform();
    }

    protected void mouseMove(String message, int offsetX, int offsetY, Locator locator, Object... args) {
        Reporter.log(message);
        new Actions(driver).moveToElement(getElement(locator, args), offsetX, offsetY).build().perform();
    }
    ...
    protected void mouseDragAndDrop(String message, WebElement elementToDrag, WebElement dragTo) {
        Reporter.log(message);
        new Actions(driver).clickAndHold(elementToDrag).moveToElement(dragTo).release().build().perform();
    }
    ...
    @Deprecated
    protected void uploadFileIE(String filePath, Locator locator, Object... args)
    {
        WebElement inputElement = getElement(locator, args);
    ...
    protected void uploadFile(String filePath, Locator uploadButton, Object... args) {

        click("Clicking on upload file button", uploadButton, args);
